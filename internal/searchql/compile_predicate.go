...
// q against a CapturedRequest. Body needles are pre-converted to []byte once
// here, not per record. Freeform terms (Field == "") match against the Tier-1
// fields available on a CapturedRequest (host, path, service, body, headers);
// correlated event fields contribute via the memory reader's orphan-event
// matching, not this predicate.
...
func evaluateTerm(t *Term, bodyNeedle []byte, r *capture.CapturedRequest) bool {
	...
	case FieldBody:
		return bytes.Contains(r.Body, bodyNeedle)
	case FieldHeaders:
...
// shape), body (substring), or any header name/value pair (substring). Method,
// status, source_ip, and correlation_id are excluded by design — typing the
// value of one of those does not return matching rows.
func MatchFreeformRequest(t *Term, bodyNeedle []byte, r *capture.CapturedRequest) bool {
	...
	if bytes.Contains(r.Body, bodyNeedle) {
		return true
	}
...
// path (indexed wildcard), or any header/body substring across the request
// and (when present) response halves.
func MatchFreeformOutboundEvent(t *Term, ev *capture.OutboundEvent) bool {
	...
	if bytes.Contains(ev.Request.Body, []byte(t.Value)) {
		return true
	}
	...
	if ev.Response != nil {
		if bytes.Contains(ev.Response.Body, []byte(t.Value)) {
			return true
		}
...
// needle. The match is case-sensitive (mirrors SQLite LIKE under
// case_sensitive_like) and operates byte-by-byte against name and each value.
// Exported so the orphan-event arm can apply the same predicate against
// ResponseEvent/OutboundEvent header maps.
...
// substring-match needle. A missing header never matches a positive predicate;
// negation is applied by the caller. Exported for the orphan-event arm.
